<template>
	<div>
		...
		<div class="form">
			...
			<text class="btn" @tap="doLogin">登 录</text>
			<div class="res">
				<text @tap="toPage('register')">用户注册</text>
				<text @tap="toPage('resetpasswd')">忘记密码</text>
			</div>
		</div>
		<!-- 第三方登录 -->
		<div class="oauth" v-if="isShowOauth">
			<text class="text">— 快速登录 —</text>
			<div class="list">
				<div @tap="oauthLogin('weixin')" v-if="showProvider.weixin" class="icon weixin"></div>
				<div @tap="oauthLogin('qq')" v-if="showProvider.qq" class="icon qq"></div>
				<div @tap="oauthLogin('sinaweibo')" v-if="showProvider.sinaweibo" class="icon sinaweibo"></div>
				<!-- <div @tap="oauthLogin('xiaomi')" v-if="showProvider.xiaomi" class="icon xiaomi"></div> -->
			</div>
		</div>
	</div>
</template>
